...
module.exports = class characterList {
    ...
    constructor(newPlaceHolderCharsOriginal, newCNameA, newCNameB, newCNameC, 
        ...
        */
        ...
        /*
        var i;
        for(i = 0; i < newCNameA.length; i++) {
            cNameA.push(newCNameA[i]);
        }
        for(i = 0; i < newCNameB.length; i++) {
            cNameB.push(newCNameB[i]);
        }
        for(i = 0; i < newCNameC.length; i++) {
            cNameC.push(newCNameC[i]);
        }
        for(i = 0; i < newCNameD.length; i++) {
            cNameD.push(newCNameD[i]);
        }
        for(i = 0; i < newCNameE.length; i++) {
            cNameE.push(newCNameE[i]);
        }
        for(i = 0; i < newCNameF.length; i++) {
            cNameF.push(newCNameF[i]);
        }
        for(i = 0; i < newCNameG.length; i++) {
            cNameG.push(newCNameG[i]);
        }
        */
    }

    generateTeamBin(min, max) {
        ...
        console.log(charArr);
        var cList = this.getMatch(charArr);
        console.log(cList);
        this.reset;
    ...
    getMatch(charArr)
    {
        ...
        {
            var getChar = 0;
            console.log("charrArr[i]" + charArr[i]);
            if(charArr[i] == 5.5)
            {
